...
class BaseTranslationModel(nn.Module):
    ...
    def __init__(self, num_words, emb_dim, hidden_dim, n_layers):
        super().__init__()
        ...
        self.vocab_projection = nn.Linear(emb_dim, num_words)
    ...
    def encode(self, inp):
        ...
        lengths = (inp != 3).to(torch.int64).sum(dim=1).clamp_max(inp.shape[1] - 1)  # take the first before padding
    ...
    def decode(self, initial_state, out_tokens):
        ...
        onehot_bos = F.one_hot(torch.full([batch_size], 2, dtype=torch.int64),
    ...
    def decode_inference(self, initial_state, max_len=100, top_p=0.8):
        ...
        state = initial_state
        outputs = [torch.full([batch_size], 2, dtype=torch.int64,
                              device=device)]
